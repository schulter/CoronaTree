...
class Tree:
    ...
    def GetAllSplits(self):
        ...
        for (u_name, v_name) in self.edgeLengths.keys():
            ...
            splitsPerEdge.update([split])
        return splitsPerEdge
    ...
    def IdentifyRepeatedSitePatterns(self):
        ...
        for site in xrange(self.sequenceLength):
            sitePattern = ""
...
class RootedTree():
    def __init__(self):
        ...
        self.host = {}
        self.numberOfLabeledVertices = 0
        self.numberOfHiddenVertices = 0        
    ...
    def SetNumberOfHiddenVerticesAndLabeledVertices(self):
        for v_name in self.vertices.keys():
            if v_name.startswith("h_"):
                self.numberOfHiddenVertices += 1
            else:
                self.numberOfLabeledVertices += 1    
...
#             if len(side) < len(leafNames) -1:        
    ...
    def OrderCharsWrtSitePatternRepeatBlocks(self,sequence):    
        char_pos_tuple = [(sequence[i], self.siteArrangedBlockwiseForEachDistinctPattern[i]) for i in xrange(self.sequenceLength)]            
    ...
    def hamDist_pattern(self,seq_u,seq_v):
        ...
        self.patternRepeats
        dist = 0.0
        for siteInd in xrange(len(self.firstSiteOfEachDistinctPattern)):
...
# Initialization   
    ...
    def InitializeNonLeafSequencesUsingMPHartigan(self):
        ...
        map(GetStatesForSite,xrange(self.sequenceLength))
    ...
    def InitializeNonLeafSequencesUsingMPUsingPatternSavings(self):
        ...
        map(GetStatesForSiteOfDistinctPattern,xrange(numberOfDistinctPatterns))       
    ...
    def ComputeExpectedAncestralStatesAndUpdateLogLikUsingPatternSavingForTransitionMatrices(self):
        ...
        def ComputeExpectedStatesAndUpdateLogLikPerSite(siteInd):
            ...
            for c in postOrderVerticesWithoutRoot:                              
                ...
                for nuc_p in xrange(4):
                    partialLikelihood_c_nuc_p = 0.0
                    for nuc_c in xrange(4):
        ...
        map(ComputeExpectedStatesAndUpdateLogLikPerSite,xrange(len(self.firstSiteOfEachDistinctPattern)))
    ...
    def ReturnLogLikForRootingAtEdge(self):        
        ...
        for u in preOrderVerticesWithoutRoot:
            ...
            for v in u.children:
                ...
                try:
                    logLik += sum(map(lambda site: log(P[DNA.index(seq_u[site]),DNA.index(seq_v[site])]), xrange(self.sequenceLength)))
                except:
                    print (P)
        return logLik
...
def GetMLVRMSTFromDistances(distanceMatrix):
    ...
    for edgeInd in xrange(len(sortedDistances)):
    ...
    for i in xrange(len(vertexObjList)):
